  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-md rounded-2xl sm:rounded-3xl border-white/20 bg-white/80 backdrop-blur-xl shadow-[0_15px_50px_rgba(4,16,58,0.08)] p-4 sm:p-6">
        ...
        >
          <DialogHeader>
            <div className="mx-auto mb-3 sm:mb-4 flex h-11 w-11 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-red-500/10">
              <XCircle className="h-5 w-5 sm:h-7 sm:w-7 text-red-500" />
            </div>
            ...
            <DialogDescription className="text-center text-sm sm:text-base text-[#1B2B4B]/60">
              ...
              <br />
              <span className="text-xs sm:text-sm text-red-500/70 mt-2 block">
          ...
          <DialogFooter className="mt-4 sm:mt-6 flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 sm:justify-center">
            ...
            <Button
              onClick={onConfirm}
              disabled={isRejecting}
              className="rounded-full bg-red-500 px-4 sm:px-6 text-sm sm:text-base text-white hover:bg-red-600 disabled:opacity-70"
            >
              {isRejecting ? (
                <>
