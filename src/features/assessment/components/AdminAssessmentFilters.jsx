...
export function AdminAssessmentFilters({
  ...
  const currentStatusLabel = PUBLISH_STATUS_OPTIONS.find(
    (opt) => opt.value === statusFilter
  )?.label || 'All Statuses';

  const currentTypeLabel = assessmentTypes.find(
    (t) => t._id === typeFilter
  )?.name || 'All Types';
      ...
      <div className="flex flex-col gap-4">
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  {assessmentTypes.map((type) => (
                    <DropdownMenuItem
                      key={type._id}
                      onClick={() => onTypeFilterChange(type._id)}
                      className={cn(
                        'rounded-xl px-4 py-2.5 text-sm cursor-pointer transition-colors',
                        typeFilter === type._id
                          ? 'bg-[#C9A84C]/10 text-[#C9A84C]'
        ...
        <div className="flex items-center justify-between border-t border-[#1B2B4B]/5 pt-3">
          <div className="text-sm text-[#1B2B4B]/50">
            {isLoading ? 'Loading...' : `${totalPages || 0} assessment${totalPages !== 1 ? 's' : ''}`}
          </div>
        </div>
      </div>
    </div>
