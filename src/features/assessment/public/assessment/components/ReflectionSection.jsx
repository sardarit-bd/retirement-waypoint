...
export const ReflectionSection = ({ domain, domainKey, value, onAnswer }) => {
  const reflectionQuestion = domain?.reflection?.question || domain?.open || '';

  if (!reflectionQuestion) return null;

  const handleChange = (e) => {
    onAnswer(`${domainKey}_open`, e.target.value);
  };
  ...
  return (
    <div className="mt-7 rounded-2xl border border-white/10 bg-white/10 p-5">
      ...
      <p className="mb-3 text-sm italic leading-7 text-white/70">{reflectionQuestion}</p>
